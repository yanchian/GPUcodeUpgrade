...
Solver::Status Solver::step() {
  float timeFlux = 0.0, timeDiffusionFlux = 0.0, timeReduce = 0.0, timeSource = 0.0, timeBCs = 0.0;
  ...
  // ============================================================
  // Strang 分裂 (2阶): source(dt/2) → fluids(dt) → source(dt/2)
  // 反应放热立即耦合到流场, 比原始顺序分裂更适合爆燃
  // ============================================================
#ifdef REACTIVE
  timeSource += source(dt * 0.5);
  timeBCs += setBoundaryConditions();
#endif
 ...
 // timeBCs = setBoundaryConditions();
  ...
  stepNumber++;
  lastDt = dt;
  std::cout << "# Step " << stepNumber << "[" << outputNumber << "]: " << std::fixed << std::setprecision(11) << u->time() << " (dt=" << std::setprecision(3) << std::scientific << dt << "). Time {hydrodynamic fluxes=" << std::fixed << std::setprecision(3) << timeFlux << "ms, diffusion fluxes =" << std::fixed << std::setprecision(3) << timeDiffusionFlux << "ms, reaction =" << timeSource << "ms, reduction=" << timeReduce << "ms, BCs=" << timeBCs << "ms}. Memory " << std::fixed << std::setprecision(0) << (float) ((totalMemory - freeMemory) >> 20) << " MiB / " << (float) (totalMemory >> 20) << " MiB" << std::endl;
